        ...
        if "EOF" in data and data["EOF"]:
            ...
            if len(self.queue) < self.queue_size :
                ...
                for i in range(len(data['linear'])):
                    ...
                    # logging.debug(f"[insert] {self.topic}: {data['linear'][i]['id']}")
                    # for q in self.queue:
                    #     print(self.queue[q].fid, end='')
                    # print()
                    
                last_ts = data.get("batch_end_ts")
                if last_ts is not None and hasattr(self.triangulation_thread, "update_heartbeat"):
                    try:
                        self.triangulation_thread.update_heartbeat(cam=self.in_name, last_ts=float(last_ts))
                    except Exception as e:
                        logging.debug(f"[Receiver] update_heartbeat error: {e}")
            else:
                logging.warning("Receiver [{}] is full.".format(self.topic))
