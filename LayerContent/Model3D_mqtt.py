...
class TriangulationThread(threading.Thread):
    def __init__(self, client, main_thread, args, settings, ks, poses, eye, dist, newcameramtx, projection_mat):
        ...
        for in_topic in input_topics:
            ...
            receiver = PointReceiver(self.client, self, self.nodename, in_name, broker, in_topic, queue_size)
...
class MainThread(threading.Thread):
    def __init__(self, client, args, settings, ks, poses, eye, dist, newcameramtx, projection_mat):
        threading.Thread.__init__(self)
        ...
        self.triangulation = TriangulationThread(self.client, self, args, settings, ks, poses, eye, dist, newcameramtx, projection_mat)
...
class MainThreadManager:
    ...
    def start_main_thread(self, date, data, mode):
        ...
        if self.main_thread is None or not self.main_thread.is_alive():
            self.main_thread = MainThread(self.mqttc, args, settings, ks, poses, eye, dist, newcameramtx, projection_mat)
            self.main_thread.start()
            if self.main_thread.is_alive():
                return_payload = json.dumps({"status": "Ready"})
            else:
                return_payload = json.dumps({"status": "Fail"})
        else:
            return_payload = json.dumps({"status": "Fail"})
    ...
    def stop_main_thread(self):
        ...
        if self.main_thread is not None:
            ...
            self.main_thread = None
            return_payload = json.dumps({"status": "Stop"})
    ...
    def data_feeder(self, file_path=f'{ROOTDIR}/LayerContent/content_output_sample.csv'):
        ...
        self.client = mqtt.Client()
        self.client.connect(*broker)
        output_topic = '/DATA/ContentDevice/LayerContent/Model3D'
        with open(file_path, 'r', newline='') as csvFile:
            ...
            for row in rows:
                ...
                self.client.publish(output_topic, json.dumps(json_str))
                print(f"[ContentLayer] Published on topic '{output_topic}': {json_str}")
        return_payload = json.dumps({"status": "Finish"})
        return return_payload
    ...
    if isinstance(data, list):
        for entry in data:
            ...
            if camera_idx is not None and camera_device and camera_fps and parameters:
                ...
                tracknet_topic = f"/DATA/{camera_device}/LayerSensing/TrackNet"
        ...
        settings['output_topic'] = '/DATA/ContentDevice/LayerContent/Model3D'
...
def main():
    ...
    for idx, serial in zip(args.camera_idxs, args.camera_device):
        input_topic = f"/DATA/{serial}/LayerSensing/TrackNet"
    ...
    mqtt_client = MqttAgent("ContentDevice", "ContentLayer")
    mqtt_client.start('140.113.213.131', 1884)
    
    # Start MainThread
    mainThread = MainThread(mqtt_client.mqttc, args, settings, ks, poses, eye, dist, newcameramtx, projection_mat)
    mainThread.start()
    mainThread.join()
