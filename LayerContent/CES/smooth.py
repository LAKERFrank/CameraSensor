...
def detectBallTypeDT(points, pred_points, v, net_front_th = 2.0, apex_height_th = 2.5, flat_diff_th = 1.0, smash_speed_th = 200.0):
    start, end = points[0], points[-1]
    # start_x, start_y, start_z = points[0].x, points[0].y, points[0].z
    # end_x, end_y, end_z = points[-1].x, points[-1].y, points[-1].z

    same_side = (start.y * end.y) >= 0
    apex_z = max(p.z for p in points)
    min_z = min(p.z for p in points)
    diff_hl = apex_z - end.z

    if v[0] == 0.0 and v[1] == 0.0 and v[2] == 0.0:
        try: 
            dt = (points[1].timestamp - start.timestamp)
            if dt <= 0:
                raise ValueError
        except Exception:
            dt = 1.0 / 120.0
        
        dx, dy, dz = (points[1].x - start.x, points[1].y - start.y, points[1].z - start.z)
        v_mps = math.sqrt(dx*dx + dy*dy + dz*dz) / dt
        speed = v_mps * 3.6
    else:
        v_mps = math.sqrt(sum(v_**2 for v_ in v))
        speed = v_mps * 3.6
    
    # y=0的z有沒有過網
    if abs(start.y) <= net_front_th:
        if same_side:
            return 'exception'
        if abs(end.y) <= net_front_th:
            return 'net'
        
        # 對手中後場
        if apex_z > apex_height_th:
            return 'lift'
        else:
            if diff_hl < flat_diff_th:
                return 'drive'
            else:
                return 'push'
    else:
