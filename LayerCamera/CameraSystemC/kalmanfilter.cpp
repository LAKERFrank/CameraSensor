...
void SimpleKalmanFilter::step(double observed_timestamp) {
    ...
    if (observed_dt > 2 * estimated_dt) {
        ...
        double ratio = 0;
        if (ema_warmup_steps_remaining_) {
            ratio = (double) ema_warmup_steps_remaining_-- / ema_warmup_steps_;
        }
        double alpha = ema_max_alpha_ * ratio + ema_min_alpha_ * (1 - ratio);
        ema_dt_ = alpha * estimated_dt + (1 - alpha) * ema_dt_;
    }
